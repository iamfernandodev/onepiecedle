...
export default function Search({ characters, winned, setCharacters }: SearchProps) {
    ...
    const handleSelectCharacter = async (characterId: string) => {
        const character = data.find(ch => ch.name === characterId) as CharacterProps;
    ...
    const handleKeyDown = (event: React.KeyboardEvent) => {
        ...
        switch (event.key) {
            case 'ArrowDown':
                if (selectedIndex < data.length - 1) setFocused(data[selectedIndex + 1].name);
                setSelectedIndex(prevIndex => (prevIndex < data.length - 1 ? prevIndex + 1 : prevIndex));
            break
            case 'ArrowUp':
                if (selectedIndex > 0) setFocused(data[selectedIndex - 1].name);
                setSelectedIndex(prevIndex => (prevIndex > 0 ? prevIndex - 1 : prevIndex));
            break
            case 'Enter':
                if (selectedIndex < 0)
                    return;
                handleSelectCharacter(data[selectedIndex].name);
            break
        }
    };
    ...
    useEffect(() => {
        const handleSearchCharacters = async(search: string) => {
            try {
                ...
                setData(
                    data.characters
                        .filter((character: CharacterProps) => !characters.map(ch => ch.name).includes(character.name))
                );
            } catch(error) {
